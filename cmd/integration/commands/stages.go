...
func stageSnapshots(db kv.TemporalRwDB, ctx context.Context, logger log.Logger) error {
	...
	_, engine, _, _, _, _ := newSync(ctx, db, nil /* miningConfig */, logger)
	chainReader := stagedsync.NewChainReaderImpl(fromdb.ChainConfig(db), tx, br, logger)
	if err := reset2.ResetBlocks(tx, db, br, bw, dirs, chainReader, engine, logger); err != nil {
...
func stageBodies(db kv.TemporalRwDB, ctx context.Context, logger log.Logger) error {
	...
	if err := db.Update(ctx, func(tx kv.RwTx) error {
		...
		if unwind > 0 {
			...
			cfg := stagedsync.StageBodiesCfg(db, nil, nil, nil, nil, nil, 0, chainConfig, br, bw)
...
func printAllStages(db kv.TemporalRwDB, ctx context.Context, logger log.Logger) error {
	sn, borSn, _, _, bscSn, _, _, _ := allSnapshots(ctx, db, logger) // ignore error here to get some stat.
	defer sn.Close()
	defer borSn.Close()
	defer bscSn.Close()
	return db.ViewTemporal(ctx, func(tx kv.TemporalTx) error { return printStages(tx, sn, borSn, bscSn) })
}
...
func allSnapshots(ctx context.Context, db kv.RoDB, logger log.Logger) (*freezeblocks.RoSnapshots, *heimdall.RoSnapshots, *dbstate.Aggregator, *freezeblocks.CaplinSnapshots, *freezeblocks.BscRoSnapshots, bridge.Store, heimdall.Store, error) {
	var err error
	...
	openSnapshotOnce.Do(func() {
		...
		_allBscSnapshotsSingleton = freezeblocks.NewBscRoSnapshots(snapCfg, dirs.Snap, logger)
	...
	return _allSnapshotsSingleton, _allBorSnapshotsSingleton, _aggSingleton, _allCaplinSnapshotsSingleton, _allBscSnapshotsSingleton, _bridgeStoreSingleton, _heimdallStoreSingleton, nil
}
...
func blocksIO(db kv.RoDB, logger log.Logger) (services.FullBlockReader, *blockio.BlockWriter) {
	openBlockReaderOnce.Do(func() {
		sn, borSn, _, _, bscSn, _, _, err := allSnapshots(context.Background(), db, logger)
		if err != nil {
			panic(err)
		}
...
func newSync(ctx context.Context, db kv.TemporalRwDB, miningConfig *buildercfg.MiningConfig, logger log.Logger) (
	...
	blockReader, blockWriter, allSn, borSn, bridgeStore, heimdallStore, _, _, err := eth.SetUpBlockReader(ctx, db, dirs, &cfg, chainConfig, dbReadConcurrency, logger, blockSnapBuildSema)
	if err != nil {
		panic(err)
	}
	...
	blockRetire := freezeblocks.NewBlockRetire(estimate.CompressSnapshot.Workers(), dirs, blockReader, blockWriter, db, blobStore, heimdallStore, bridgeStore, chainConfig, &cfg, notifications.Events, blockSnapBuildSema, logger)

	stageList := stages2.NewDefaultStages(context.Background(), db, blobStore, p2p.Config{}, &cfg, sentryControlServer, notifications, nil, blockReader, blockRetire, nil, nil,
...
func initConsensusEngine(ctx context.Context, cc *chain2.Config, dir string, db kv.RwDB, blockReader services.FullBlockReader, bridgeStore bridge.Store, heimdallStore heimdall.Store, logger log.Logger) consensus.Engine {
	...
	return ethconsensusconfig.CreateConsensusEngine(ctx, &nodecfg.Config{Dirs: datadir.New(dir)}, cc, consensusConfig, config.Miner.Notify, config.Miner.Noverify,
		config.WithoutHeimdall, config.DisableBlobPrune, blockReader, db.ReadOnly(), logger, polygonBridge, heimdallService)
}
